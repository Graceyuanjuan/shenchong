...
export class BehaviorStrategyManager {
     ...
     */
    setupHotReload() {
        this.dbAdapter.onStrategiesChanged((strategies) => {
     ...
     */
    convertStrategyRecordToBehaviorRule(record) {
        try {
            return {
                id: record.id,
                name: record.name,
                description: record.description || record.name,
                state: record.conditions.states,
                emotion: record.conditions.emotions,
     ...
     * T5-A: 转换策略条件
     */
    convertStrategyConditions(conditions) {
        ...
        return async (context) => {
            try {
                ...
                if (action.nextActions) {
                    result.nextActions = action.nextActions.map((nextAction) => ({
     ...
     * 获取匹配的策略
     */
    getMatchingStrategies(state, emotion, context) {
     ...
     * 检查策略条件
     */
    checkConditions(strategy, context) {
     ...
     * 更新执行统计
     */
    updateExecutionStats(strategyId, duration, success, error) {
     ...
     * 保存策略到数据库
     */
    async saveStrategyToDB(strategy) {
        try {
     ...
     * 从数据库删除策略
     */
    async removeStrategyFromDB(strategyId) {
        try {
     ...
     * T4-C: 注册视觉反馈管理器
     */
    registerVisualFeedbackManager(manager) {
     ...
     */
    convertBehaviorRuleToStrategyRecord(rule) {
     ...
     * T4-C: 策略执行时触发视觉反馈
     */
    triggerStrategyVisualFeedback(strategy, context, results) {
        if (!this.visualFeedbackManager || !this.rhythmManager) {
            return;
        }
        // 根据策略类型和执行结果选择视觉反馈类型
        let visualCue = VisualCueType.IDLE_PULSE;
        ...
        if (this.rhythmManager) {
            ...
            this.rhythmManager.processBehavior(mockBehavior, {
                state: context.state,
                emotion: context.emotion.currentEmotion,
 ...
 * 创建默认的行为策略管理器实例
 */
export function createBehaviorStrategyManager() {
    return new BehaviorStrategyManager();
}
export default BehaviorStrategyManager;
//# sourceMappingURL=BehaviorStrategyManager.js.map