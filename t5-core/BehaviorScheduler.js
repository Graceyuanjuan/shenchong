...
import { StrategyManager } from './BehaviorStrategy';
import { BehaviorRhythmManager } from '../modules/rhythm/BehaviorRhythmManager';
import { RhythmMode } from '../types/BehaviorRhythm';
import { createRhythmAdaptationEngine } from '../modules/rhythm/RhythmAdaptationEngine';
// 行为类型定义
export var BehaviorType;
(function (BehaviorType) {
    // 基础行为
    BehaviorType["IDLE_ANIMATION"] = "idle_animation";
    BehaviorType["HOVER_FEEDBACK"] = "hover_feedback";
    BehaviorType["AWAKEN_RESPONSE"] = "awaken_response";
    BehaviorType["CONTROL_ACTIVATION"] = "control_activation";
    // 情绪行为
    BehaviorType["EMOTIONAL_EXPRESSION"] = "emotional_expression";
    BehaviorType["MOOD_TRANSITION"] = "mood_transition";
    // 插件行为
    BehaviorType["PLUGIN_TRIGGER"] = "plugin_trigger";
    BehaviorType["PLUGIN_CALLBACK"] = "plugin_callback";
    // 交互行为
    BehaviorType["USER_PROMPT"] = "user_prompt";
    BehaviorType["SYSTEM_NOTIFICATION"] = "system_notification";
    // 延时行为
    BehaviorType["DELAYED_ACTION"] = "delayed_action";
    BehaviorType["ANIMATION_SEQUENCE"] = "animation_sequence";
})(BehaviorType || (BehaviorType = {}));
/**
 * 行为调度器类
 */
export class BehaviorScheduler {
    constructor(emotionEngine, pluginRegistry) {
        this.activeBehaviors = new Map();
        this.behaviorQueue = [];
        this.scheduledBehaviors = new Map();
        this.sessionId = '';
        ...
        if (emotionEngine)
            console.log(`🧠 EmotionEngine integrated`);
        if (pluginRegistry)
            console.log(`🔌 PluginRegistry integrated`);
     ...
     */
    async schedule(state, emotion, context) {
     ...
     * 传统行为调度方法（向后兼容）
     */
    async scheduleLegacy(state, emotion, context) {
        const startTime = Date.now();
        // 构建执行上下文
     ...
     * 查找匹配的行为规则
     */
    findMatchingBehaviors(state, emotion, context) {
        // 从规则映射表中获取匹配的行为
     ...
     * 执行行为列表
     */
    async executeBehaviors(behaviors, context) {
        const executed = [];
     ...
     * 执行单个行为
     */
    async executeBehavior(behavior, context) {
     ...
     * 计算下次调度时间
     */
    calculateNextScheduleTime(state, emotion) {
        // 基础间隔
     ...
     * 节拍回调 - 在节拍器节拍时调用
     */
    onRhythmTick(timestamp, interval) {
        // 可以在这里添加基于节拍的行为逻辑
        // 比如定期检查状态变化、触发周期性行为等
     ...
     * 添加行为规则（向后兼容）
     */
    addBehaviorRule(state, emotion, behavior) {
        if (!this.behaviorRules[state]) {
            this.behaviorRules[state] = {};
        }
...
//# sourceMappingURL=BehaviorScheduler.js.map