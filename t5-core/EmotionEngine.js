...
import { AIEmotionDriverFactory } from '../modules/AIEmotionDriver';
// T5-C: 导入节奏上下文类型
import { getCurrentTimeOfDay, UserActivityLevel } from '../types/rhythm/RhythmContext';
export class EmotionEngine {
    constructor(aiDriver) {
        ...
        this.emotionLogs = [];
        // T5-C: 节奏上下文回调
        this.rhythmContextCallbacks = new Set();
        this.currentPetState = PetState.Idle;
     ...
     * 基于用户意图更新情绪
     */
    updateEmotionFromIntent(intent) {
        const triggers = [];
        // 根据意图类型确定触发因素
        switch (intent.type) {
            case 'screenshot':
     ...
     * 将用户情绪映射到宠物情绪
     */
    mapUserEmotionToPetEmotion(userEmotion) {
        switch (userEmotion) {
     ...
     * 应用情绪规则
     */
    applyEmotionRule(ruleId, triggers) {
     ...
     * 混合情绪（不完全替换，而是融合）
     */
    blendEmotion(emotion, intensity, duration) {
     ...
     * 基于任务结果更新情绪
     */
    updateEmotionFromTaskResult(success, taskType) {
        if (success) {
     ...
     * 根据情绪推荐状态切换
     */
    recommendStateTransition(currentState) {
     ...
     */
    updateEmotionByState(state, context) {
        try {
            // 使用 AI 情绪驱动器决定新情绪
     ...
     * T5-B: 记录情绪变化日志
     */
    recordEmotionLog(emotion, state, context, trigger) {
        const log = {
            timestamp: Date.now(),
            emotion,
            state,
    ...
    getEmotionStatistics() {
        return {
            aiDriverStats: this.aiEmotionDriver.getStatistics?.() || {},
     ...
     * T5-C: 注册节奏上下文更新回调
     */
    onRhythmContextUpdate(callback) {
     ...
     * T5-C: 移除节奏上下文更新回调
     */
    offRhythmContextUpdate(callback) {
