...
import { PetBrainBridge, PetBrainBridgeConfig } from './PetBrainBridge';
...
export class PetBrain {
  ...
  // PetBrainBridge 集成
  private petBrainBridge: PetBrainBridge;
  
  // 配置
  private config: PetBrainConfig;
  ...
  private stateTransitionCallbacks: Map<PetState, Function[]> = new Map();
  ...
  async processInput(input: string, context: any = {}): Promise<{
    ...
    try {
      ...
      // 5. 记录对话
      this.stateMemory.recordConversation(
        input,
        primaryIntent,
        response.message || 'Action completed',
        response.emotion || this.emotionEngine.getCurrentEmotion().currentEmotion,
        response.success
      );
      ...
      this.stateMemory.recordConversation(
        input,
        { type: 'error', confidence: 0, parameters: {}, rawInput: input, timestamp: Date.now() },
        errorMessage,
        EmotionType.Calm,
        false
      );
   ...
   * 执行意图
   */
  private async executeIntent(intent: UserIntent, context: any): Promise<PluginResponse> {
    // 获取支持该意图的插件
    ...
    try {
      ...
      const response = await this.pluginRegistry.executePlugin(
        selectedPlugin.id, 
        intent, 
        {
          ...context,
      ...
      if (availablePlugins.length > 1) {
        for (let i = 1; i < availablePlugins.length; i++) {
          try {
            ...
            return await this.pluginRegistry.executePlugin(
              fallbackPlugin.id, 
              intent, 
              context
            );
   ...
   * 处理未知意图
   */
  private async handleUnknownIntent(input: string): Promise<{
    response: string;
    emotion: EmotionType;
    nextState?: PetState;
  }> {
   ...
   * 处理没有找到插件的情况
   */
  private async handleNoPluginFound(intent: UserIntent): Promise<PluginResponse> {
   ...
   */
  async transitionToState(newState: PetState): Promise<void> {
   ...
   * 处理状态变化的核心逻辑
   */
  public handleStateChange(newState: PetState): void {
    const oldState = this.currentState;
   ...
   * 执行状态转换回调
   */
  private async executeStateTransitionCallbacks(state: PetState): Promise<void> {
  ...
  public getStateStatistics(): {
    ...
    // 1. 计算状态频率
    const stateFrequency: Record<PetState, number> = {
      [PetState.Idle]: 0,
      [PetState.Hover]: 0,
      [PetState.Awaken]: 0,
      [PetState.Control]: 0
    ...
    Object.entries(stateFrequency).forEach(([state, count]) => {
      if (count > maxCount) {
        maxCount = count;
        mostFrequentState = state as PetState;
      }
    });
   ...
   * 判断输入是否像命令
   */
  private isCommandLike(input: string): boolean {
   ...
   */
  async enterIdleState(): Promise<void> {
   ...
   */
  async enterHoverState(): Promise<void> {
   ...
   */
  async enterAwakenState(): Promise<void> {
   ...
   */
  async enterControlState(): Promise<void> {
   ...
   * 设置默认状态转换回调
   */
  private async setupDefaultStateCallbacks(): Promise<void> {
    console.log('📞 Setting up default state transition callbacks');
    
    // 静碗状态回调
    this.onStateTransition(PetState.Idle, async (state: PetState, emotion: any) => {
      console.log(`💤 Idle state callback: emotion=${emotion.currentEmotion}`);
      // 可以在这里添加静态状态的特定逻辑
    });
    
    // 感应状态回调
    this.onStateTransition(PetState.Hover, async (state: PetState, emotion: any) => {
      console.log(`✨ Hover state callback: emotion=${emotion.currentEmotion}`);
      // 可以在这里添加悬浮提示逻辑
    });
    
    // 唤醒状态回调
    this.onStateTransition(PetState.Awaken, async (state: PetState, emotion: any) => {
      console.log(`🌟 Awaken state callback: emotion=${emotion.currentEmotion}`);
      // 可以在这里添加工具激活逻辑
    });
    
    // 控制状态回调
    this.onStateTransition(PetState.Control, async (state: PetState, emotion: any) => {
   ...
   */
  async dispatchEmotionBehavior(emotion: EmotionType): Promise<void> {
   ...
   */
  async dispatchEventBehavior(eventName: string): Promise<void> {
   ...
   */
  async updateEmotionWithBehaviorDispatch(emotion: EmotionType, intensity: number = 0.7): Promise<void> {
   ...
   */
  async resetBehaviorSystem(): Promise<void> {
