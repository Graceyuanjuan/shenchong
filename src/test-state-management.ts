...
async function testStateManagement() {
  ...
  console.log('=== 完整状态统计 ===');
  console.log(`当前状态: ${finalStats.currentState}`);
  ...
  console.log('状态频率统计:');
  Object.entries(finalStats.stateFrequency).forEach(([state, count]) => {
...
async function testEnhancedStateTransition() {
  ...
  // 注册示例插件
  const { ScreenshotPlugin, NotePlugin } = await import('./plugins/ExamplePlugins');
