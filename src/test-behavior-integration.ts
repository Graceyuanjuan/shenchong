...
import { createBehaviorSchedulingManager, EventDrivenBehaviorScheduler, BehaviorScheduleEvent } from './core/BehaviorSchedulingIntegrator';
import { BaseBehaviorStrategy, StrategyContext, IBehaviorStrategy } from './core/BehaviorStrategy';
import { BehaviorType, BehaviorDefinition } from './core/BehaviorScheduler';
...
async function testEventDrivenScheduler() {
  ...
  // 监听事件
  eventScheduler.on(BehaviorScheduleEvent.SCHEDULE_STARTED, (data: any) => {
    console.log('📡 [Event] 调度开始:', data.state, data.emotion);
  });
  
  eventScheduler.on(BehaviorScheduleEvent.SCHEDULE_COMPLETED, (data: any) => {
