...
describe('BehaviorRhythmManager', () => {
  ...
  describe('基础功能测试', () => {
    it('应该正确初始化默认状态', () => {
      ...
      expect(state.currentMode).toBe(RhythmMode.STEADY);
      expect(state.tickCount).toBe(0);
      expect(state.config.mode).toBe(RhythmMode.STEADY);
    });
    ...
    it('应该正确切换节奏模式', () => {
      ...
      const state = rhythmManager.getCurrentState();
      expect(state.currentMode).toBe(RhythmMode.PULSE);
      expect(state.config.mode).toBe(RhythmMode.PULSE);
    });
  ...
  describe('节拍回调测试', () => {
    it('应该正确注册和调用节拍回调', (done) => {
      ...
      rhythmManager.setRhythmMode(RhythmMode.STEADY, { baseInterval: 100 });
      rhythmManager.start();
  ...
  describe('节奏模式测试', () => {
    it('STEADY模式应该产生相对稳定的间隔', (done) => {
      ...
      rhythmManager.tick(callback);
      rhythmManager.setRhythmMode(RhythmMode.STEADY, { 
        baseInterval: 100,
        variation: 0.1 
      });
      rhythmManager.start();
    ...
    it('SEQUENCE模式应该按照预设序列循环', (done) => {
      ...
      rhythmManager.tick(callback);
      rhythmManager.setRhythmMode(RhythmMode.SEQUENCE, { 
        baseInterval: 200,
        sequence: sequence
      });
      rhythmManager.start();
  ...
  describe('自适应模式测试', () => {
    it('应该根据情绪强度调整节拍间隔', () => {
      rhythmManager.setRhythmMode(RhythmMode.ADAPTIVE, { 
        baseInterval: 1000
      });
  ...
  describe('同步模式测试', () => {
    it('应该支持外部同步', () => {
      ...
      const state = rhythmManager.getCurrentState();
      expect(state.currentMode).toBe(RhythmMode.SYNC);
  ...
  describe('默认配置测试', () => {
    it('应该有有效的默认配置', () => {
      ...
      expect(devConfig.mode).toBe(RhythmMode.STEADY);
      expect(devConfig.baseInterval).toBeGreaterThan(1000); // 开发环境应该较慢
      expect(devConfig.intensity).toBe(RhythmIntensity.LOW);
    });
  });
});
